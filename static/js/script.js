document.addEventListener("DOMContentLoaded", () => {
    const runBtn = document.getElementById('runBtn'),
          ...
          input = document.getElementById('tickerSearch'),
          drop = document.getElementById('autocomplete-dropdown');
    ...
    // --- RUN BUTTON LOGIC (UPDATED) ---
    if(runBtn) runBtn.addEventListener('click', async () => {
        loader.classList.add('active');
        
        // 1. Get values explicitly
        let seqLenVal = parseInt(document.getElementById('seqLen').value) || 60;
        let horizonVal = parseInt(document.getElementById('horizon').value) || 30;

        // 2. Enforce Limits (Double check for safety)
        if(seqLenVal > 365) seqLenVal = 365;
        if(seqLenVal < 10) seqLenVal = 10;
        
        if(horizonVal > 90) horizonVal = 90;
        if(horizonVal < 1) horizonVal = 1;
        ...
        try {
            const res = await fetch('/api/predict', { 
                ...
                body:JSON.stringify({
                    ...
                    seq_len: seqLenVal,   // Sending corrected Integer
                    horizon: horizonVal   // Sending corrected Integer
                }) 
            });
        ...
        chart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                    ...
                    { label:'AI Fit', data:valData, borderColor:'#FBA24F', borderWidth:2, pointRadius:0, borderDash:[4,4], tension:0.3, fill:false },
